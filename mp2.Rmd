...
committee_summary <- function(data_trans, cmt_name)
  data_trans %>%
    ...
    summarise(cont_to_ici = sum(transaction_amt, na.rm = TRUE))
>>>>>>> 95742c8a41e14a0521241c14fc03db50778d5118
```
```{r}
#global variables
col1 <- "#EBDF8E"
...
ggplot(susanb_cand_fund, aes(x = state, y = cont_to_ici,
                             fill = cand_ici, label = cont_to_ici))+
  coord_flip()+
  ggtitle("Susan B Anthony List Inc. Candidate Fund")+
  geom_bar(stat = "identity", position = "stack") + 
  geom_text(size = 3, position = position_stack(vjust = 0.5)) +
  scale_x_discrete("State") +
  scale_y_continuous("Total '24E' Transactions:\n    Contributions advocating the election of a candidate", limits = c(0,10000), breaks = seq(0,10000, 5000), labels = c("$0", "$5,000", "$10,000"))+
  scale_fill_manual(values = c(col1, col2, col3),
                    name = "Candidate\n   Status",
=======
>>>>>>> 6c94d1b5db9ccc6e8798662f9cce0ecf262ed520
...
ggplot(contributions, aes(x = state, y = cont_to_ici,
                             fill = cand_ici))+
  coord_flip()+
  ggtitle(com_name)+
  geom_bar(stat = "identity") + 
